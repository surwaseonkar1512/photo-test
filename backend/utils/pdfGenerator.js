...
export const generatePDF = async (data, outputPath) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (data.logo) {
                try {
                    const response = await axios.get(data.logo, { responseType: 'arraybuffer' });
                    doc.image(Buffer.from(response.data), 50, 45, { width: 80 });
                } catch (e) {
                    // Fallback to text if logo fails
                    doc.fontSize(20).font('Helvetica-Bold').text(data.studioName || 'VISIONARY STUDIO', 50, 50);
                }
            } else {
                doc.fontSize(22).font('Helvetica-Bold').text(data.studioName || 'VISIONARY STUDIO', 50, 50);
            }
            ...
            // Event Details
            doc.fontSize(10).font('Helvetica-Bold').text('EVENT INFORMATION', 50, 200);
            ...
            if (data.services && data.services.length > 0) {
                data.services.forEach(service => {
                    ...
                    doc.fontSize(10).font('Helvetica').text(service, 65, yPos);
                    yPos += 20;
                });
            }
            ...
            doc.fontSize(12).font('Helvetica-Bold').text('PRICING DETAILS', 50, yPos);
            yPos += 25;
            ...
            const drawRow = (label, value, isBold = false) => {
                doc.font(isBold ? 'Helvetica-Bold' : 'Helvetica').fontSize(10).text(label, 60, yPos);
                doc.text(value.toLocaleString(), 480, yPos, { align: 'right', width: 60 });
                yPos += 22;
            };

            const discount = data.actualPrice - data.sellingPrice;
            drawRow('Actual Menu Price', data.actualPrice);
            if (discount > 0) {
            ...
            drawRow('Subtotal', data.sellingPrice, true);
            ...
            doc.fontSize(16).text(`₹ ${data.finalAmount.toLocaleString()}`, 400, yPos, { align: 'right', width: 140 });

            // --- 5. TERMS & CONDITIONS ---
            yPos += 60;
            doc.fontSize(10).font('Helvetica-Bold').text('TERMS & NOTES', 50, yPos);
            yPos += 20;
            const terms = [
            ...
            doc.fontSize(10).font('Helvetica-Bold').fillColor(primaryColor).text('THANK YOU FOR CHOOSING VISIONARY STUDIO!', 50, footerY + 20, { align: 'center', width: 500 });
